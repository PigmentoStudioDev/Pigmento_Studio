...
export async function up({ db, payload, req }: MigrateUpArgs): Promise<void> {
  await db.run(sql`CREATE TABLE \`payload_mcp_api_keys\` (
  	...
  	\`projects_delete\` integer DEFAULT false,
  	\`proposals_find\` integer DEFAULT false,
  	\`proposals_create\` integer DEFAULT false,
  	\`proposals_update\` integer DEFAULT false,
  	\`proposals_delete\` integer DEFAULT false,
...
export async function down({ db, payload, req }: MigrateDownArgs): Promise<void> {
  ...
  `)
  await db.run(sql`INSERT INTO \`__new_payload_locked_documents_rels\`("id", "order", "parent_id", "path", "users_id", "media_id", "projects_id", "proposals_id") SELECT "id", "order", "parent_id", "path", "users_id", "media_id", "projects_id", "proposals_id" FROM \`payload_locked_documents_rels\`;`)
